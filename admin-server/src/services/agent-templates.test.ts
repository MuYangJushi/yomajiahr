...
test("listAgentTemplates：仓库源含全部内置模板（#47；source 字段已下线，署名集中在 NOTICE.md）", () => {
  // 回退仓库源：临时指向空 STATE_DIR，强制走 repoTemplatesDir()。
  const emptyState = join(tmpdir(), `yomajiahr-templates-clawmax-${process.pid}`);
  process.env.OPENCLAW_STATE_DIR = emptyState;
  const list = listAgentTemplates();
  // 上游移植 25 个 + 内置 2 个 = 27（允许后续新增，至少 27）
  assert.ok(list.length >= 27, `仓库源应含全部内置模板，实际 ${list.length}`);
  // 抽样：engineer 模板字段齐全且 role=employee
