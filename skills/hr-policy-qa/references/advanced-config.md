...
- **检索**：员工侧 `hr-policy-qa` 经 `knowledge_search`（FastGPT 唯一知识源，无本地 `memory_search` 回退）。FastGPT 不可用时如实告知不可用、不编造。
...
- 导入与文档管理见 [hr-admin skill](../../hr-admin/SKILL.md) 与 [admin-operations.md](../../hr-admin/references/admin-operations.md)：聊天经 `knowledge_import`，web 经 Admin Portal「知识库」页（列表/切片预览/删除/新建库 + 审计）。
