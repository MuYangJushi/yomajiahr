...
// 核心保证：用实际部署的 seed 组合（空 agents、空 bindings、空 channels）跑生成器，
// 产出的 openclaw.json 必须通过 validate-config —— 否则空平台一上线 gateway 就拒绝起。
// 直接读仓库里被改动的 seed 文件，确保测的是真实部署内容，而非内存 fixture。
// 2026-06-22：channels.json 收口为 []（真正全空白起步，渠道账号也由用户在「渠道」页自建）。
...
test('空白起步 seed：无 agent / 无 binding / 无 channel → 生成器产出合法 openclaw.json', () => {
  ...
  assert.equal(store.channels.length, 0, 'seed channels 应为空（渠道账号由用户在渠道页自建）');
